...
#+BEGIN_SRC sh
for file in vonal*shp; do v.in.ogr -o input=$file output=v;v.clean -c input=v output=vc tool=snap,break threshold=1.0,0; v.out.ogr -s input=vc type=line output=new_$file format=ESRI_Shapefile; g.remove -f typ=vec name=v,vc; done
#+END_SRC
